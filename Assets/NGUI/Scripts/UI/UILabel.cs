...
#region SLG
    ...
    [HideInInspector][NonSerialized] public bool isArabic;
    [HideInInspector][NonSerialized] public bool supportEmoji;
    [HideInInspector][NonSerialized] private Material emojiMaterial;
    ...
    {
        base.Awake();
        // SLG: load perfered font based on language
...
// 		            if (emojiMaterial == null)
// 		            {
// 		                emojiMaterial = new Material(Shader.Find("Unlit/Text"));
//                         var emojiTexture = ResManager.Instance.GetTexture(ResManager.EmojiAtlas);
//                         emojiMaterial.SetTexture("_MainTex2", emojiTexture);
//                     }
//
//                     emojiMaterial.mainTexture = mTrueTypeFont.material.mainTexture;
...
// #endregion
	...
	// public UnicodeText processedText
	// {
	// 	get
	// 	{
	...
	public void ProcessText (bool legacyMode = false, bool full = true)
	{
		...
		// {
  //           mProcessedText.Parse("", supportEmoji);
		// 	return;
		// }
...
// #region SLG
// 			    var source = new UnicodeText(printedText, supportEmoji);
//
// 				// Wrap the text
// 				bool fits = NGUIText.WrapText(source, out mProcessedText, true, false,
// 					mOverflowEllipsis && mOverflow == Overflow.ClampContent);
//
// 				if (Arabic || isArabic)
//                 {
//                     mProcessedText.ToArabic(supportEmoji);
//                 }
// #endregion
	...
	public int GetCharacterIndexAtPosition (Vector2 localPos, bool precise)
	{
		if (isValid)
		{
			// if (string.IsNullOrEmpty(processedText.text)) return 0;

			UpdateNGUIText();

			// if (precise) NGUIText.PrintExactCharacterPositions(mProcessedText, mTempVerts, mTempIndices);
			// else NGUIText.PrintApproximateCharacterPositions(mProcessedText, mTempVerts, mTempIndices);
	...
	public string GetWordAtCharacterIndex (int characterIndex)
	{
		...
		if (characterIndex != -1 && characterIndex < s.Length)
		{
			...
			// 		return NGUIText.StripSymbols(new UnicodeText(word, supportEmoji)).text;
			// 	}
			// }
		}
		return null;
	...
	// 	int offset = NGUIText.CalculateOffsetToFit(new UnicodeText(text, supportEmoji));
	// 	NGUIText.bitmapFont = null;
	// 	NGUIText.dynamicFont = null;
	// 	return offset;
	// }
	...
	// public bool Wrap (UnicodeText text, out UnicodeText final) { return Wrap(text, out final, 1000000); }
	//
	// /// <summary>
	// /// Convenience function -- wrap the current text given the label's settings and the given height.
	// /// </summary>
	//
	// public bool Wrap (UnicodeText text, out UnicodeText final, int height)
	// {
	// 	UpdateNGUIText();
	...
	public void UpdateNGUIText ()
	{
		...
		NGUIText.fontScale = isDynamic ? mScale : ((float)mFontSize / mFont.defaultSize) * mScale;
	    NGUIText.supportEmoji = supportEmoji;

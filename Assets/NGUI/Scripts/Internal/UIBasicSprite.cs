...
#region Fill Functions
	...
	protected void Fill (List<Vector3> verts, List<Vector2> uvs, List<Color> cols, Rect outer, Rect inner)
	{
		...
		if (DrawMirrorType.None != _mirrorType)
		{
			switch (_mirrorType)
			{
				case DrawMirrorType.Horizontal:
					AddHMirror(verts, uvs, cols);
					break;

				case DrawMirrorType.Vertical:
					AddVMirror(verts, uvs, cols);
					break;

				case DrawMirrorType.Both:
					AddHMirror(verts, uvs, cols);
					AddVMirror(verts, uvs, cols);
					break;
			}
	...
	[SerializeField]
	private DrawMirrorType _mirrorType = DrawMirrorType.None;
	public DrawMirrorType MirrorType { get { return _mirrorType; } }

	[SerializeField]
	private bool _offsetInPixel = false;
	public bool OffsetInPixel { get { return _offsetInPixel; } }

	[SerializeField]
	private Vector2 _mirrorOffset = Vector2.zero;

	[SerializeField]
	[Range(0f, 0.5f)]
	private float _mirrorHOffset = 0f;

	[SerializeField]
	[Range(0f, 0.5f)]
	private float _mirrorVOffset = 0f;

	void AddHMirror(List<Vector3> verts, List<Vector2> uvs, List<Color> cols)
	{
		int orgCount = verts.Count;
		orgCount /= 4;
		orgCount *= 4;

		float a1 = _offsetInPixel ? _mirrorOffset.x : _mirrorHOffset * width, a2 = 0;
		switch (pivot)
		{
			case Pivot.Left:
			case Pivot.TopLeft:
		...
		// LB -> LT -> RT -> RB
		Vector3 v;
		for (int index = orgCount - 1; -1 < index; index--)
		{
			v = verts[index];
	...
	void AddVMirror(List<Vector3> verts, List<Vector2> uvs, List<Color> cols)
	{
		int orgCount = verts.Count;
		orgCount /= 4;
		orgCount *= 4;

		float a1 = _offsetInPixel ? _mirrorOffset.y : _mirrorVOffset * height, a2 = 0;
		switch (pivot)
		{
			case Pivot.Top:
			case Pivot.TopLeft:
	...
	[SerializeField]
	protected bool _drawRound = false;
	public bool IsRound { get { return _drawRound; } }

	[SerializeField]
	protected CustomRoundType _customRoundType = CustomRoundType.SimpleShortFit;
	public CustomRoundType RoundType { get { return _customRoundType; } }
	...
	void SimpleFill (List<Vector3> verts, List<Vector2> uvs, List<Color> cols)
	{
		...
		if (_drawRound)
		{
			switch (_customRoundType)
			{
				case CustomRoundType.Transformed:
					DoRoundFill(verts, uvs, cols, true, true);
					break;

				case CustomRoundType.SimpleLongFit:
					DoRoundFill(verts, uvs, cols, false, false);
					break;

				case CustomRoundType.SimpleShortFit:
					DoRoundFill(verts, uvs, cols, false, true);
					break;
			}
		}
		else
		{
			DoSimpleFill(verts, uvs, cols);
		}
	}

	void DoSimpleFill (List<Vector3> verts, List<Vector2> uvs, List<Color> cols)
	{
		Vector4 v = drawingDimensions;
		Vector4 u = drawingUVs;
		Color gc = drawingColor;
	...
	void DoRoundFill (List<Vector3> verts, List<Vector2> uvs, List<Color> cols, bool transformed, bool fitShort)
	{
		Color gc = drawingColor;

		Vector4 v = drawingDimensions;
		float rvx = 0.5f * (v.z - v.x);
		float rvy = 0.5f * (v.w - v.y);
		float centervx = v.x + rvx;
		float centervy = v.y + rvy;
		if (!transformed)
		{
			float r;
			if (fitShort) { r = Mathf.Min(rvx, rvy); }
			else { r = Mathf.Max(rvx, rvy); }
		...
		Vector4 u = drawingUVs;
		float rux = 0.5f * (u.z - u.x);
		float ruy = 0.5f * (u.w - u.y);
		float centerux = u.x + rux;
		float centeruy = u.y + ruy;

		Vector3 tv = Vector3.zero;
		Vector2 tu = Vector2.zero;

		float step = Mathf.PI * 2 / _pointCount;
		float tr =  Mathf.Deg2Rad * (_angleOffset - 90) + step;
		float tc, ts;
		for (int index = 0; index < _pointCount; index++)
		{
			...
			tc = Mathf.Cos(tr);
			ts = Mathf.Sin(tr);
			tv.x = centervx + rvx * tc;
			tv.y = centervy + rvy * ts;
			tu.x = centerux + rux * tc;
			tu.y = centeruy + ruy * ts;
			verts.Add(tv);
			uvs.Add(tu);
			cols.Add(gc);

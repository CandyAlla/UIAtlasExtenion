...
static public class NGUIText
{
    ...
    public static bool supportEmoji;
...
#region SLG
	...
	// static public BMSymbol GetSymbol (UnicodeText text, int index, int textLength)
	// {
	//     if (supportEmoji)
	//     {
	//         return UnicodeParser.GetSymbol(text[index]);
	// }
	//     else if (useSymbols)
	//     {
 //            return (bitmapFont != null) ? bitmapFont.MatchSymbol(text.text, index, textLength) : null;
 //        }
 //
	//     return null;
	// }
	...
	// [System.Diagnostics.DebuggerHidden]
	// [System.Diagnostics.DebuggerStepThrough]
	// static public Color ParseColor (UnicodeText text, int offset = 0) { return ParseColor24(text, offset); }
	//
	// /// <summary>
	// /// Parse a RrGgBb color encoded in the string.
	// /// </summary>
	...
	// [System.Diagnostics.DebuggerHidden]
	// [System.Diagnostics.DebuggerStepThrough]
	// static public Color ParseColor24 (UnicodeText text, int offset = 0)
	// {
	// 	int r = (NGUIMath.HexToDecimal((char)text[offset])     << 4) | NGUIMath.HexToDecimal((char)text[offset + 1]);
	// 	int g = (NGUIMath.HexToDecimal((char)text[offset + 2]) << 4) | NGUIMath.HexToDecimal((char)text[offset + 3]);
	// 	int b = (NGUIMath.HexToDecimal((char)text[offset + 4]) << 4) | NGUIMath.HexToDecimal((char)text[offset + 5]);
	// 	float f = 1f / 255f;
	...
	// [System.Diagnostics.DebuggerHidden]
	// [System.Diagnostics.DebuggerStepThrough]
	// static public Color ParseColor32 (UnicodeText text, int offset)
	// {
	// 	int r = (NGUIMath.HexToDecimal((char)text[offset]) << 4) | NGUIMath.HexToDecimal((char)text[offset + 1]);
	// 	int g = (NGUIMath.HexToDecimal((char)text[offset + 2]) << 4) | NGUIMath.HexToDecimal((char)text[offset + 3]);
	// 	int b = (NGUIMath.HexToDecimal((char)text[offset + 4]) << 4) | NGUIMath.HexToDecimal((char)text[offset + 5]);
	// 	int a = (NGUIMath.HexToDecimal((char)text[offset + 6]) << 4) | NGUIMath.HexToDecimal((char)text[offset + 7]);
	// 	float f = 1f / 255f;
	...
	// /// </summary>
	//
	// static public bool ParseSymbol (UnicodeText text, ref int index)
	// {
	// 	int n = 1;
	// 	bool bold = false;
	// 	bool italic = false;
	...
	[System.Diagnostics.DebuggerHidden]
	[System.Diagnostics.DebuggerStepThrough]
	static public bool IsHex (int ch)
	{
	...
	// static public bool ParseSymbol (UnicodeText text, ref int index, BetterList<Color> colors, bool premultiply,
 ...
 //
	// 			default:
	// 			{
	// 				int ch0 = text[index + 1];
 //                    int ch1 = text[index + 2];
 //
	// 				if (IsHex(ch0) && IsHex(ch1))
	// 				{
	// 					int a = (NGUIMath.HexToDecimal((char)ch0) << 4) | NGUIMath.HexToDecimal((char)ch1);
	// 					mAlpha = a / 255f;
	// 					index += 4;
	// 					return true;
	// 				}
 ...
 //
	// 		if (EncodeColor24(c) != text.Substring(index + 1, 6).text.ToUpper())
	// 			return false;
 //
 ...
 //
	...
	// 		if (EncodeColor32(c) != text.Substring(index + 1, 8).text.ToUpper())
	// 			return false;
 //
	// 		if (colors != null)
	// 		{
 ...
 //
	...
	// /// </summary>
 //
	// static public UnicodeText StripSymbols (UnicodeText text)
	// {
	// 	if (text != null)
	// 	{
 ...
 //
	...
	// [DebuggerHidden]
	// [DebuggerStepThrough]
	// static public void EndLine (ref UnicodeTextBuilder s)
	// {
	// 	int i = s.Length - 1;
	...
	//[DebuggerHidden]
	//[DebuggerStepThrough]
	//static void ReplaceSpaceWithNewline (ref UnicodeTextBuilder s)
	//{
	//	int i = s.Length - 1;
	//	if (i > 0 && IsSpace(s[i])) s[i] = '\n';
	//}
	...
	// static public Vector2 CalculatePrintedSize (UnicodeText text)
	// {
	// 	var v = Vector2.zero;
 //
 ...
 //
	...
	// static public int CalculateOffsetToFit (UnicodeText text)
	// {
	// 	if (string.IsNullOrEmpty(text.text) || regionWidth < 1) return 0;
 //
	// 	Prepare(text.text);
 //
 ...
 //
	...
	// /// </summary>
 //
	// static public UnicodeText GetEndOfLineThatFits (UnicodeText text)
	// {
	// 	int textLength = text.Length;
 ...
 //
	...
	// /// </summary>
 //
	// static public bool WrapText (UnicodeText text, out UnicodeText finalText, bool wrapLineColors = false)
	// {
 ...
 //
	...
	// /// </summary>
 //
	// static public bool WrapText (UnicodeText text, out UnicodeText finalText, bool keepCharCount, bool wrapLineColors, bool useEllipsis = false)
	// {
	// 	if (regionWidth < 1 || regionHeight < 1 || finalLineHeight < 1f)
	// 	{
	// 		finalText = new UnicodeText();
	// 		return false;
	// 	}
 //
 ...
 //            finalText = new UnicodeText();
	...
	// 	}
 //
	// 	if (string.IsNullOrEmpty(text.text)) text.Parse(" ");
 //
	// 	var sb = new UnicodeTextBuilder();
	// 	int textLength = text.Length;
	// 	Prepare(text.text);
 //
 ...
 //
 //            if (ch >= 44032 && ch <= 55215) eastern = false;
 //
 ...
 //
	// 	if (start < offset) sb.Append(text.Substring(start, offset - start));
	// 	if (wrapLineColors && mColors.size > 0) sb.Append("[-]");
	// 	finalText = sb.ToUnicodeText(text.codes != null);
	// 	mColors.Clear();
 ...
 //
	...
	// /// </summary>
 //
	// static public void Print (UnicodeText text, List<Vector3> verts, List<Vector2> uvs, List<Color> cols)
	// {
 ...
 //
	...
	// 		{
 //                //
 //                //No Correct Font Metrics in DynamicFont
 //                // Leading    X
 //                // Ascent     V
 //                // Baseline   X
 //                // Descent    X
 //                //
 //                // Aligned with Baseline, but FontSize * 0.5
 //                //
 //                
 //                float center = symbol.height * scale - fontSize * fontScale;
 //
 //                //v0x = x + symbol.offsetX * scale;
 //                //v1x = v0x + symbol.width * scale;
 //                //v1y = -(y + symbol.offsetY * scale);
 //                //v0y = v1y - symbol.height * scale ;
 //
 //                v0x = x;
 //                v1x = v0x + symbol.width * scale;
 //                v0y = -y - symbol.height * scale + center * 0.5f;
 //                v1y = -y + center * 0.5f;
 //         
	// 			var w = symbol.advance * scale;
 //
 ...
 //
	// 				int offsetU = supportEmoji ? 2 : 0;
	// 				float u0x = uv.xMin + offsetU;
	// 				float u0y = uv.yMin;
	// 				float u1x = uv.xMax + offsetU;
	// 				float u1y = uv.yMax;
 //
 ...
 //
	...
	// /// </summary>
 //
	// static public void PrintApproximateCharacterPositions (UnicodeText text, List<Vector3> verts, List<int> indices)
	// {
 ...
 //
	...
	// /// </summary>
 //
	// static public void PrintExactCharacterPositions (UnicodeText text, List<Vector3> verts, List<int> indices)
	// {
	// 	if (string.IsNullOrEmpty(text.text)) text.Parse(" ");
 //
	// 	Prepare(text.text);
 ...
 //
	...
	// /// </summary>
 //
	// static public void PrintCaretAndSelection (UnicodeText text, int start, int end, List<Vector3> caret, List<Vector3> highlight)
	// {
	// 	if (string.IsNullOrEmpty(text.text)) text.Parse(" ");
 //
	// 	Prepare(text.text);

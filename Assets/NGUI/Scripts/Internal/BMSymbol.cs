...
#region SLG
    public float GetAdvanceWidth(float fontScale)
    {
        return mAdvance * fontScale * scale;
    }
#endregion
...
#region SLG
                    mAdvance = mSprite.width + (mSprite.paddingLeft + mSprite.paddingRight) + (int)Math.Ceiling((scale - 1) * 4);
#endregion

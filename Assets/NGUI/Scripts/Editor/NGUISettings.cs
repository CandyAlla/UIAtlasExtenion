...
#region Convenience accessor properties
	#region texutre setting

	static public Vector2 GetTextureMax
	{
		get
		{
			Vector2 xy = Vector2.zero;
			xy.x = GetInt("NGUI Texture Max X", 2048);
			xy.y = GetInt("NGUI Texture Max y", 2048);
			return xy;
		}
		set
		{
			SetInt("NGUI Texture Max X",(int)value.x);
			SetInt("NGUI Texture Max y",(int)value.y);
		}
	}


	static public bool IsSpriteMipmap
	{
		get
		{
			return GetBool("NGUI Sprite Open Mipmap", false);
		}
		set
		{
			SetBool("NGUI Sprite Open Mipmap",value);
		}
	}
...
#region SLG
    static public int alphaThreshold
    {
        get { return GetInt("Alpha Threshold", 10); }
        set { SetInt("Alpha Threshold", value); }
    }
#endregion

...
public class UICenterOnChild : MonoBehaviour
{
    ...
    public bool centerRestrain = false;
	...
	public void Recenter ()
    {
        ...
        {
            closest = nextCenter;
            CenterOn(closest, panelCenter, false);
            nextCenter = null;
        }
        else
        {
	...
	void CenterOn (Transform target, Vector3 panelCenter, bool scrollTo = true)
	{
		if (target != null && mScrollView != null && mScrollView.panel != null)
		{
...
#endif
            {
                if(centerRestrain)
                {
                    Vector3 prePosition = mScrollView.transform.localPosition;
                    Vector2 preCo = mScrollView.panel.clipOffset;

                    mScrollView.transform.localPosition = mScrollView.transform.localPosition - localOffset;
                    Vector2 nowCo = mScrollView.panel.clipOffset;
                    nowCo.x += localOffset.x;
                    nowCo.y += localOffset.y;
                    mScrollView.panel.clipOffset = nowCo;

                    Bounds b = mScrollView.bounds;
                    Vector3 constraint = mScrollView.panel.CalculateConstrainOffset(b.min, b.max);

                    mScrollView.transform.localPosition = prePosition;
                    mScrollView.panel.clipOffset = preCo;
                ...
                {
                    panelTrans.localPosition = panelTrans.localPosition - localOffset;
                    Vector2 co = mScrollView.panel.clipOffset;
                    co.x += localOffset.x;
                    co.y += localOffset.y;
                    mScrollView.panel.clipOffset = co;
                }
    ...
    public void CenterOn (Transform target, bool scrollTo = true)
	{
		if (mScrollView != null && mScrollView.panel != null)
		{
			Vector3[] corners = mScrollView.panel.worldCorners;
			Vector3 panelCenter = (corners[2] + corners[0]) * 0.5f;
			CenterOn(target, panelCenter, scrollTo);
		}
        else
        {
            nextCenter = target;
